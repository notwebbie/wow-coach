...
C_StableInfo = { GetNumStableSlots = function() return 2 end }
function UnitExists(u) return u == "pet" end
local realUnitName = UnitName
function UnitName(u) if u == "pet" then return "Snarlfang" end return realUnitName(u) end
...
local rr = WoWCoachProbeDB.runs[1].results
assert(rr["pet.happiness"].status == "ok" and tostring(rr["pet.happiness"].value):find("C_PetInfo happiness=3"),
       "pet happiness should read via C_PetInfo: " .. tostring(rr["pet.happiness"] and rr["pet.happiness"].value))
assert(tostring(rr["pet.trainingPoints"].value):find("spent=40"), "training points should read")
assert(tostring(rr["pet.foodTypes"].value):find("Meat"), "food types should read")
print("smoke3: pet path OK — " .. tostring(rr["pet.happiness"].value))
